CREATE OR REPLACE VIEW observability.season_health AS
WITH game_coverage AS (
    SELECT
        ...
        COUNT(*) FILTER (
            WHERE NOT EXISTS (SELECT 1 FROM events e WHERE e.game_id = g.game_id)
              AND EXISTS (
                  SELECT 1
...
CREATE OR REPLACE VIEW observability.dataset_health AS
WITH totals AS (
    SELECT
...
COMMENT ON COLUMN observability.season_health.acknowledged_gap_games IS
    'Completed games without events that backfill has classified as done or skipped';

COMMENT ON COLUMN observability.season_health.actionable_gap_games IS
    'Completed games without events that remain eligible for ingestion';
