...
SELECT g.game_id, g.season, g.game_type,
       ...
       CASE
           ...
           WHEN stored.shift_rows > 0 THEN 'loaded'
...
CROSS JOIN LATERAL (
    SELECT count(*) AS shift_rows FROM shifts s WHERE s.game_id = g.game_id
) stored
...
COMMENT ON VIEW observability.shift_game_coverage IS
    'Stored snapshots take precedence over latest fetch outcomes. Absence of a fetch record does not establish that a game was never attempted. Counts use the existing game-leading shift primary key.';
